...
class HapiFhirServerTest implements LibraryHelper, ResourceFileUtil {
  ...
  void buildLibraryBundle() {
    ...
    String globalCommonsFunctionsCql =
        getStringFromTestResource("/includes/GlobalCommonFunctions.cql");
  ...
  void createResource() {
    ...
    Object when = hapiClient.create().resource(any(Library.class)).execute();

    when((Object) when).thenReturn(outcome);
